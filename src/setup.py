...
# Порядок везде один — по тому, насколько больную задачу функция снимает
# (владелец, 25.09.2026): СКЛЕЙКА экономит артисту 3–10 тысяч за сведение,
# ОТБОР даёт пост с именем, ВКЛАДЫШ снимает «у друга другая площадка»,
# СЛЕЖУ — пропущенный релиз и концерт, ДВОЙНИК и ПРОЯВКА — любопытство.
...
BOT_DESCRIPTION = (
    ...
    # Название — строкой, описание — под ним (владелец, 25.09.2026). Жирного
    # описание не умеет, а в 512 знаков Telegram (эмодзи — за два) влезает впритык.
    "🎛 СКЛЕЙКА\nПришли вокал и бит — сведу их в готовый трек.\n\n"
    "🎙 ОТБОР\nПишешь сам? Пришли трек — он выйдет в канале с твоим именем.\n\n"
    "📼 ВКЛАДЫШ\nКинь ссылку на трек — пришлю карточку со всеми площадками для друга.\n\n"
    "🔔 СЛЕЖУ\nНазови артистов и город — напишу о релизе или концерте.\n\n"
    "🪞 ДВОЙНИК\nНа сколько процентов твоя музыка совпадает с артистами.\n\n"
    "🎞 ПРОЯВКА\nНазови артиста или песню — расскажу, откуда это взялось.\n\n"
...
BOT_SHORT = "Вокал и бит — склею в готовый трек. Свой трек пришли — выйдет в канале @plenka_fm."
...
CHANNEL_DESCRIPTION = (
    "Откуда взялся звук?\n\n"
    "Бот {bot}: пришли свой трек — выйдет в канале. Пришли вокал и бит — склеит в готовый трек. "
    "И разберёт любого артиста или песню."
)
...
🎛 <b>СКЛЕЙКА</b>
Пришли вокал и бит — бот сведёт их в готовый трек: MP3, WAV для площадок и ролик ДО/ПОСЛЕ для сторис. Выложишь — присылай в отбор.
...
СКЛЕЙКА. Пришли вокал и бит — бот сведёт их в готовый трек: MP3, WAV для площадок и ролик ДО/ПОСЛЕ для сторис. Выложишь — присылай в отбор.
...
def announce_buttons() -> list[list[dict]]:
    ...
    # СКЛЕЙКА вместо ПРОЯВКИ (владелец, 24.09.2026): в закрепе — то, ради чего пишут музыку.
    return (telegram.url_button("🎙 Прислать свой трек", bot_link("pin"))
            + telegram.url_button("🎛 Склеить вокал и бит", bot_link("skleyka_pin"))
