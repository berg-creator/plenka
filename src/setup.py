...
def check() -> int:
    ...
    # Платная реакция звёздами не в счёт: ею голосуют за деньги, а нам нужен
    # тот, кто просто ткнул 🔥. Отсутствие поля целиком означает «все».
    reactions = info.get("available_reactions")
    emoji = [r for r in (reactions or []) if r.get("type") == "emoji"]
    if reactions is not None and not emoji:
        print("  ✗ реакции выключены")
        todo.append(
            "Включи реакции: канал → Управление → Реакции → Все.\n"
            "     Через API это не делается (метода нет в Bot API), только руками.\n"
            "     Без них не работает голосование реакциями в конце поста,\n"
            "     а реакция — самое дешёвое действие читателя."
        )
    else:
        print(f"  ✓ реакции включены ({len(emoji) or 'все'})")
