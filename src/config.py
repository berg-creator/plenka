...
# Заходы срочного: утро (ночные новости), день и вечер. Событие, случившееся
# в обед, не должно ждать до завтрашнего утра. Запускает их дежурство
...
# выходом раз в час (publish --releases), а не в четыре обычных слота, и ждёт
# полный трек от владельца RELEASE_TRACK_WAIT_HOURS. Со звуком за московские
# сутки — не больше RELEASE_LOUD_PER_DAY постов о релизах: остальные уходят
# молча, а обычные посты в такой день уступают ленту (publish.due). Цель
# для пятницы с девятью релизами — около девяти постов, со звуком три-четыре.
...
# RELEASE_LOUD_PER_DAY не идёт. Молчит любой пост, не только о релизе: крон GitHub
# опаздывает на часы и может увести вечерний слот за полночь.
...
# Обычные посты очереди — по московским часам: утренняя дорога, обед, конец
# рабочего дня и вечер. Выпускает их дежурство (moderate.publish_shift) раз
