...
# Свежесть релизов — решение владельца от 11.09.2026: «вышел альбом» пятидневной
# давности уже не новость. Пост о релизе пишется сразу при находке (compose --fresh
# в запусках urgent.yml) и встаёт вперёд очереди, пока релизу не больше
# RELEASE_FRESH_HOURS (publish.next_post). Старше RELEASE_STALE_DAYS релиз
# в РЕЛИЗ не берётся, а готовый пост убирается из очереди. В пятницу выходит
# до девяти релизов, четыре слота за двое суток пропускают восемь — остальное
# протухает, и так задумано.
RELEASE_STALE_DAYS = 3
RELEASE_FRESH_HOURS = 48
