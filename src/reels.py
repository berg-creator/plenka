...
лицензирован для неё. Добавляется он только в приложении на телефоне: с компьютера
YouTube и ВКонтакте его не накладывают. Первым выбором идёт трек героя ролика:
Shorts показывают ролик тем, кто этот звук уже слушал. В Telegram при загрузке звука не добавить — поэтому
...
# Мерить надо не к голосу, а к тому, что звучит на склейке: склейки стоят в паузах,
# и верх микса там почти пуст (-46 дБ выше 1,5 кГц на skleyka-3). Блик на -15
# выскакивал над ним на 20 дБ, удар на -11 гремел под битом — владелец 25.09:
# «эффект в переходе очень громкий». Теперь блик вровень с верхом паузы, удар — на 1–2 дБ над битом.
CHIME_DB = -30.0
BOOM_DB = -19.0
...
# Действие и выгода одной строкой, в безопасную ширину SAFE_TEXT: «пришли свой трек
...
# на «с» и «ш». Сила де-эссера — на слух владельца (23.09.2026): при 0,4 фраза
# с тремя свистящими подряд («Чиф Киф записал») звучала съеденной, при 0,15
# свист оставался. Полка — с 4,5 кГц, где разборчивость, а не выше, в «воздухе»,
...
VOICE_CHAIN = (
    ...
    "highshelf=f=4500:g=2.5,deesser=i=0.22"
)
...
# её назвал владелец: отзвук 10% и дилей 7%, а послушав — втрое меньше.
REVERB_SHARE = 0.10 / 3
DELAY_SHARE = 0.07 / 3
...
# и только пока голос звучит, а бочка и 808 играют ровно. На -13 владелец бита
# всё равно не услышал: громкость бита держит 808, а её телефон не играет. На -10
# бит был на 1 дБ громче голоса по R128 (в СКЛЕЙКЕ вровень, VOCAL_OVER_BEAT = 0): голос ведёт
# за счёт места в своих полосах, а не за счёт тихого бита. Со своими битами
# владельца (own-bit1..3) -10 оказалось громко: 26.09 «биты чуть тише» — минус 2 дБ,
# бит на 1 дБ тише голоса. С битами Pixabay 26.09 «всё-таки чуть тише» — ещё минус 2,
# бит на 3 дБ тише голоса. Выбран Extendo, «ещё на децибел» — на 4 дБ; в 13:36 «ещё тише» — на 6 дБ тише голоса.
BEAT_LUFS = -17.0
...
def deliver(script: dict, video: Path, cover: Path) -> None:
    ...
    # С 23.09.2026 бит вшит во все четыре версии, и звук площадки поверх не нужен: подпись
    # «без музыки, добавь в приложении» после этого врала владельцу (замечено 26.09).
    # У передачи (`music`: null) бита нет нигде: своя музыка — отбивки в фоне зала,
    # трек героя добавляется ради страницы звука, без громкости (только в приложении:
    # с компьютера звук из библиотеки не накладывается, docs/research/2026-09-21/zvuk-pri-zagruzke.md).
    beat = bed(script) is not None
    music = ("бит уже в ролике, звук площадки не добавляй" if beat
...
def _selftest() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # около -50 LUFS — вровень с верхом микса в паузе, а не над ним.
        ...
        assert 1.5 < loudest[0] < 2.0 and -52 < loudest[1] < -48, loudest
