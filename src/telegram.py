...
def send_video_url(
    ...
    Сниппет из паблика так не отправить: файл с t.me по ссылке Telegram
    не забирает («failed to get HTTP URL content»), его качает
    telegram_web.preview_video и заливает send_video_file.
    """
    payload: dict[str, Any] = {
        "chat_id": chat_id,
        "video": url,
