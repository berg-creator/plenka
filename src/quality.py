...
# законен и сюда не попадает: ловится первое лицо рядом с прослушиванием.
#
# Без местоимения ловятся только устойчивые обороты о себе: «дослушал до конца»,
# «не снимаю с повтора», «поставил на будильник». Форма у них общая с третьим
# лицом («он дослушал до конца»), но в посте канала так говорят о себе, и ровно
# эти реплики стояли образцом в voice.md, пока владелец 22.09.2026 не велел
# их убрать — одна такая ушла в вердикт 11.09.
INVENTED_LISTENING = re.compile(
    ...
    r"\b(?:до|пере)слуша\w+\s+(?:до\s+конца|\w+\s+раз\w*)|"
    r"\b(?:не\s+)?снима\w+\s+с\s+повтора\b|\bс\s+повтора\s+не\s+снима\w+|"
    r"\bпостав\w+\s+на\s+(?:повтор|репит|будильник)\b"
)
...
def _selftest() -> None:
    ...
    # Реплика о себе без «я»: ровно такие стояли образцом в voice.md до 22.09.2026.
    # Тот же оборот вопросом к читателю остаётся законным (проверен выше).
    for line in ("Дослушал до конца. Второй раз не стал.",
                 "Неделю не снимаю с повтора.",
                 "Поставил на будильник и не пожалел."):
        impersonal = f"<b>SODA LUV ВЫПУСТИЛ АЛЬБОМ</b>\n\nДевять вещей без гостей.\n\n{line}"
        assert any(p.startswith("выдуманное прослушивание") for p in problems(impersonal, "verdict", single)), \
            (line, problems(impersonal, "verdict", single))
